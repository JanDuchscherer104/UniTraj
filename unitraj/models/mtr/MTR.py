...
from .MTR_utils import PointNetPolylineEncoder, build_mlps, get_batch_offsets
...
class MotionTransformer(BaseModel):
    ...
    def __init__(self, config):
        ...
        self.model_cfg.MOTION_DECODER["CENTER_OFFSET_OF_MAP"] = self.model_cfg[
            "center_offset_of_map"
        ]
        self.model_cfg.MOTION_DECODER["NUM_FUTURE_FRAMES"] = self.model_cfg[
            "future_len"
        ]
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            output["predicted_trajectory"] = (
                out_dists  # [B, c, T, 5] to be able to parallelize code
            )
        else:
            output["predicted_probability"] = out_dict["pred_scores"]  # #[B, c]
            output["predicted_trajectory"] = out_dict[
                "pred_trajs"
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer, lr_lbmd, last_epoch=-1, verbose=True
        )
...
class MTRDecoder(nn.Module):
        ...
        if use_place_holder:
            ...
            for cur_type in self.object_type:
                ...
                cur_intention_points = (
                    torch.from_numpy(cur_intention_points).float().view(-1, 2)
                )
    ...
    def apply_dynamic_map_collection(
        ...
        dynamic_dist = dynamic_dist.min(dim=-1)[
            0
    ...
    def get_decoder_loss(self, tb_pre_tag=""):
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            loss_reg_vel = F.l1_loss(
                pred_vel, center_gt_trajs[:, :, 2:4], reduction="none"
            )
            loss_reg_vel = (
                (loss_reg_vel * center_gt_trajs_mask[:, :, None])
                .sum(dim=-1)
                .sum(dim=-1)
            )
    ...
    def get_dense_future_prediction_loss(
        ...
        loss_reg_vel = F.l1_loss(
            pred_dense_trajs_vel, obj_trajs_future_state[:, :, :, 2:4], reduction="none"
        )
        loss_reg_vel = (
        ...
        fake_scores = pred_dense_trajs.new_zeros(
            (num_center_objects, num_objects)
        ).view(
            -1, 1
        ...
        temp_gt_idx = torch.zeros(
            num_center_objects * num_objects
        ).long()  # (num_center_objects * num_objects)
        temp_gt_trajs = (
    ...
    def forward(self, batch_dict):
        ...
        obj_feature = obj_feature.new_zeros(
        ...
        map_feature = map_feature.new_zeros(
