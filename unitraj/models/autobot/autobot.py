...
class AutoBotConfig(BaseModelConfig["AutoBotEgo"]):
    ...
    tx_hidden_size: int = Field(default=384)
    ...
    grad_clip_norm: float = Field(default=5.0)

    max_num_agents: int = Field(default=15)
    map_range: float = Field(default=100.0)
    max_num_roads: int = Field(default=256)
    max_points_per_lane: int = Field(default=20)
    manually_split_lane: bool = Field(default=False)
    point_sampled_interval: int = Field(default=1)
    num_points_each_polyline: int = Field(default=20)
    vector_break_dist_thresh: float = Field(default=1.0)

    # map_attr: int
    k_attr: int = Field(default=2)
    map_attr: int = Field(default=2)
...
class MapEncoderPts(nn.Module):
    ...
    def get_road_pts_mask(self, roads):
        ...
        road_pts_mask = road_pts_mask.masked_fill(
            (road_pts_mask.sum(-1) == roads.shape[2]).unsqueeze(-1), False
    ...
    def forward(self, roads, agents_emb):
        ...
        road_pts_feats = (
            self.road_pts_lin(roads[:, :, :, : self.map_attr])
            .view(B * S, P, -1)
            .permute(1, 0, 2)
        )
...
class OutputModel(nn.Module):
    ...
    def forward(self, agent_decoder_state):
        ...
        pred_obs = self.observation_model(
            agent_decoder_state.reshape(-1, self.d_k)
        ).reshape(T, BK, -1)
...
class AutoBotEgo(BaseModel):
    ...
    def __init__(self, config: AutoBotConfig):
        ...
        for _ in range(self.L_enc):
            ...
            self.social_attn_layers.append(
                nn.TransformerEncoder(tx_encoder_layer, num_layers=1)
            )
        ...
        # ============================== MAP ENCODER ==========================
        self.map_encoder = MapEncoderPts(
        ...
        # ============================== AutoBot-Ego DECODER ==============================
        self.Q = nn.Parameter(
            torch.Tensor(self.T, 1, self.c, self.d_k), requires_grad=True
        )
        ...
        self.P = nn.Parameter(
            torch.Tensor(self.c, 1, self.d_k), requires_grad=True
        )  # Appendix C.2.
    ...
    def process_observations(self, ego, agents):
        ...
        env_masks = (
            env_masks.unsqueeze(1).repeat(1, self.c, 1).view(ego.shape[0] * self.c, -1)
        )
    ...
    def temporal_attn_fn(self, agents_emb, agent_masks, layer):
        ...
        temp_masks = temp_masks.masked_fill(
            (temp_masks.sum(-1) == T_obs).unsqueeze(-1), False
        )
    ...
    def social_attn_fn(self, agents_emb, agent_masks, layer):
        ...
        agents_soc_emb = layer(
            agents_emb, src_key_padding_mask=agent_masks.view(-1, num_agents)
        )
        agents_soc_emb = agents_soc_emb.view(num_agents, B, T_obs, -1).permute(
            2, 1, 0, 3
        )
    ...
    def _forward(self, inputs):
        ...
        orig_map_features, orig_road_segs_masks = self.map_encoder(
            roads, ego_soctemp_emb
        )
        map_features = (
        ...
        mode_probs = F.softmax(
            self.prob_predictor(mode_params_emb).squeeze(-1), dim=0
        ).transpose(0, 1)

        # return  [c, T, B, 5], [B, c]
        output = {}
        output["predicted_probability"] = mode_probs  # #[B, c]
        output["predicted_trajectory"] = out_dists.permute(
            2, 0, 1, 3
        )  # [c, T, B, 5] to [B, c, T, 5] to be able to parallelize code
    ...
    def forward(self, batch):
        ...
        agents_in, agents_mask, roads = (
            inputs["obj_trajs"],
            inputs["obj_trajs_mask"],
            inputs["map_polylines"],
        )
        ego_in = torch.gather(
            agents_in,
            1,
            inputs["track_index_to_predict"]
            .view(-1, 1, 1, 1)
            .repeat(1, 1, *agents_in.shape[-2:]),
        ).squeeze(1)
        ego_mask = torch.gather(
            ...
            inputs["track_index_to_predict"]
            .view(-1, 1, 1)
            .repeat(1, 1, agents_mask.shape[-1]),
        ).squeeze(1)
        ...
        roads = torch.cat(
            [
                inputs["map_polylines"][..., :2],
                inputs["map_polylines_mask"].unsqueeze(-1),
            ],
            dim=-1,
        )
    ...
    def get_loss(self, batch, prediction):
        ...
        loss = self.criterion(
            prediction, ground_truth, inputs["center_gt_final_valid_idx"]
        )
        return loss
...
class Criterion(nn.Module):
    ...
    def nll_pytorch_dist(self, pred, data, mask, rtn_loss=True):
        ...
        if rtn_loss:
            ...
            return ((-biv_gauss_dist.log_prob(data_reshaped)).sum(-1) * mask).sum(
                1
            )  # Laplace
        else:
            # return (-biv_gauss_dist.log_prob(data)).sum(-1)  # Gauss
            ...
            return ((-biv_gauss_dist.log_prob(data_reshaped)).sum(dim=2) * mask).sum(
                1
            )  # Laplace
    ...
    def nll_loss_multimodes(self, output, data, center_gt_final_valid_idx):
        ...
        with torch.no_grad():
            for kk in range(modes):
                nll = self.nll_pytorch_dist(
                    pred[kk].transpose(0, 1), data, mask, rtn_loss=False
                )
                log_lik_list.append(
                    -nll.unsqueeze(1)
                )  # Add a new dimension to concatenate later
        ...
        for kk in range(modes):
            nll_k = (
                self.nll_pytorch_dist(
                    pred[kk].transpose(0, 1), data, mask, rtn_loss=True
                )
                * post_pr[:, kk]
            )
            loss += nll_k.mean()
