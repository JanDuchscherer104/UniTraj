...
# Add DatasetItem import for type checking/compat
...
def merge_batch_by_padding_2nd_dim(tensor_list, return_pad_mask=False):
    ...
    for k in range(len(tensor_list)):
        ...
        assert (
            cur_tensor.shape[2] == num_feat1 and cur_tensor.shape[3] == num_feat2
    ...
    ret_tensor = torch.cat(
        ret_tensor_list, dim=0
    )  # (num_stacked_samples, num_feat0_maxt, num_feat1, num_feat2)
...
def count_valid_steps_past(mask):
    ...
    idx_of_first_zero = np.where(reversed_mask == 0)[
        0
...
def get_kalman_difficulty(output):
    """
    Compute kalman difficulty at 2s, 4s, and 6s for each sample.
    Accepts List[DatasetItem] or List[dict].
    If the gt future is not valid up to the considered second, the difficulty is set to -1.
    """
    for data_sample in output:
        # unify access for DatasetItem vs dict
        if isinstance(data_sample, DatasetItem):
            past_trajectory = data_sample.obj_trajs[0, :, :2]
            past_mask = data_sample.obj_trajs_mask[0, :]
            gt_future = data_sample.obj_trajs_future_state[0, :, :2]
...
def classify_track(
    start_point, end_point, start_velocity, end_velocity, start_heading, end_heading
):
...
def interpolate_polyline(polyline, step=0.5):
    ...
    new_polyline = np.concatenate(
        (new_polyline, np.zeros((new_polyline.shape[0], 1))), axis=1
    )
    return new_polyline
...
def get_trajectory_type(output):
    for data_sample in output:
        ...
        if isinstance(data_sample, DatasetItem):
            ...
            # read past mask and state similarly below
            assert data_sample.obj_trajs_mask[0, -1]
            start_point = data_sample.obj_trajs[0, -1, :2]
            start_velocity = data_sample.obj_trajs[0, -1, -4:-2]
            start_heading = 0.0
            end_heading = get_heading(
                data_sample.obj_trajs_future_state[0, : valid_end_point + 1, :2]
            )[-1]
        else:
            valid_end_point = int(data_sample["center_gt_final_valid_idx"])
            end_point = data_sample["obj_trajs_future_state"][0, valid_end_point, :2]
...
class DynamicSampler(Sampler):
    ...
    def set_sampling_strategy(self, sampleing_dict):
        ...
        for k, v in sampleing_dict.items():
            ...
            sampled_data_idx = np.random.choice(
                data_idx, data_num, replace=False
            ).tolist()
