...
class DatasetConfig(BaseConfig["BaseDataset"]):
    ...
    load_num_workers: int = Field(0, description="Number of workers for loading data")
    ...
    overwrite_cache: bool = Field(False, description="Overwrite existing cache")
    store_data_in_memory: bool = Field(False, description="Store data in memory")
...
class BaseDataset(Dataset):
    ...
    def __init__(self, config: DatasetConfig, is_validation=False):
        self.config = config
    ...
    def load_data(self):
        ...
        for cnt, data_path in enumerate(self.config.paths.data):
            phase, dataset_name = data_path.split("/")[-2], data_path.split("/")[-1]
            self.cache_path = Path(self.config.cache_path) / dataset_name / phase
            ...
            if self.config.use_cache or is_ddp():
                ...
                if self.cache_path.exists() and not self.config.overwrite_cache:
                    print(
                        f"Warning: cache path {self.cache_path} already exists, skip "
                    )
                    file_list = self.get_data_list(data_usage_this_dataset)
                else:
                    ...
                    data_splits = [
                        (data_path, mapping, list(data_splits[i]), dataset_name)
                        for i in range(process_num)
                    ]
                    # save the data_splits in a tmp directory
                    temp_dir = self.config.paths.temp_dir / dataset_name / phase
                    temp_dir.mkdir(parents=True, exist_ok=True)
                    assert isinstance(temp_dir, Path)
                    for i in range(process_num):
                        with temp_dir.with_stem(f"{i}.pkl").open("wb") as f:
                            pickle.dump(data_splits[i], f)

                    # results = self.process_data_chunk(0)
                    with Pool(processes=process_num) as pool:
                        results = pool.map(
                            self.process_data_chunk, list(range(process_num))
                        )
                    ...
                    with open(self.cache_path / "file_list.pkl", "wb") as f:
    ...
    def process_data_chunk(self, worker_index) -> Dict[str, Any]:
        """
        Process a chunk of data files in parallel.

        Args:
            worker_index (int): Index of the worker/process.

        Returns:
            Dict[str, Any]: Mapping of group names to file info for processed data.
        """
        with open(Path("tmp") / f"{worker_index}.pkl", "rb") as f:
            data_chunk = pickle.load(f)
        file_list = {}
        data_path, mapping, data_list, dataset_name = data_chunk
        hdf5_path = self.cache_path / f"{worker_index}.h5"
        ...
        with h5py.File(hdf5_path, "w") as f:
            for cnt, file_name in enumerate(data_list):
                ...
                for i, record in enumerate(output):
                    ...
                    file_info["h5_path"] = str(hdf5_path)
    ...
    def preprocess(self, scenario: RawScenarioDict) -> InternalFormatDict:
        """
    ...
    def __getitem__(self, idx: int) -> DatasetItem:
        ...
        record = {
            k: (
                ...
                if group[k].dtype.type == np.bytes_
                else group[k][()]
            )
            for k in group.keys()
        }
    ...
    def get_data_list(self, data_usage):
        file_list_path = Path(self.cache_path) / "file_list.pkl"
    ...
    def get_agent_data(
        ...
        Args:
            ...
            obj_trajs_past (np.ndarray['num_objects, num_timestamps, box_dim', float32]): Past trajectories.
            obj_trajs_future (np.ndarray['num_objects, num_timestamps, box_dim', float32]): Future trajectories.
            track_index_to_predict (np.ndarray['num_center_objects', int]): Indices of agents to predict.
            sdc_track_index (int): Index of the self-driving car.
            timestamps (np.ndarray['num_timestamps', float32]): Timestamps for each step.
            obj_types (np.ndarray['num_objects', int]): Object types.
    ...
    def get_interested_agents(
        ...
        """
        Select valid center agents for prediction.
        ...
        Returns:
            Tuple[Optional[np.ndarray], Optional[np.ndarray]]: (center_objects, selected_indices)
        """
        center_objects_list = []
    ...
    def transform_trajs_to_center_coords(
        ...
        Returns:
            np.ndarray: Transformed trajectories.
        """
    ...
    def get_map_data(
        ...
        """
        Prepare map polyline features relative to each center agent.

        Args:
            center_objects (np.ndarray['num_center_objects, 10', float32]): Center agent states.
            map_infos (MapInfosDict): Map information.

        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: (map_polylines, map_polylines_mask, map_polylines_center)
        """
    ...
    def get_manually_split_map_data(
        ...
        if len(batch_polylines) > num_of_src_polylines:
            ...
            polyline_center = np.sum(batch_polylines[:, :, 0:2], axis=1) / np.clip(
                np.sum(batch_polylines_mask, axis=1)[:, None].astype(float),
                a_min=1.0,
                a_max=None,
            )
            # Convert the center_offset to a numpy array and repeat it for each object
            center_offset_rot = np.tile(
                np.array(center_offset, dtype=np.float32)[None, :],
                (num_center_objects, 1),
            )

            center_offset_rot = common_utils.rotate_points_along_z(
                points=center_offset_rot[:, None, :], angle=center_objects[:, 6]
            )
            ...
            map_polylines_mask = np.pad(
                ...
                (
                    (0, 0),
                    (0, num_of_src_polylines - map_polylines_mask.shape[1]),
                    (0, 0),
                ),
            )
        ...
        temp_sum = (
            map_polylines[:, :, :, 0:3]
            * map_polylines_mask[:, :, :, None].astype(np.float32)
        ).sum(
            axis=-2
        )  # (num_center_objects, num_polylines, 3)
        map_polylines_center = temp_sum / np.clip(
            map_polylines_mask.sum(axis=-1)[:, :, np.newaxis].astype(float),
            a_min=1.0,
            a_max=None,
        )
    ...
    def trajectory_filter(self, data: InternalFormatDict) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Dictionary of tracks to predict.
        """
